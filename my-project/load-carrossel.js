...
(async function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      const checkFirebase = setInterval(function () {
        ...
        if (Date.now() - startedAt >= timeoutMs) {
          clearInterval(checkFirebase);
          reject(new Error('Firebase Firestore indisponível para carregar o carrossel.'));
        }
      }, 100);
    });
  }
  ...
  function setupCarousel(rootSelector, fotos) {
    ...
    fotos.forEach(function (foto, index) {
      ...
      const isHeroCarousel = rootSelector === '.bannerSwiper';
      image.loading = isHeroCarousel && index === 0 ? 'eager' : 'lazy';
      if (isHeroCarousel && index === 0) {
  ...
  try {
    ...
    const heavySection = document.querySelector('.heavy-carousel-section');
    if (!heavySection || typeof window.IntersectionObserver !== 'function') {
